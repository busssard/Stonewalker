...
## Feature Sprint (February 2026)

### Completed (7 tasks, 4 parallel agents, ~20 min wall-clock)

**1. Shop Button Auto-Hide**
- Context processor at `source/app/context_processors.py` provides `shop_visible` and `user_count`
- `SHOP_VISIBLE_USER_THRESHOLD = 1000` in both dev/prod settings
- Nav template conditionally renders Shop link: `{% if shop_visible %}`
- Tests in `main/tests/test_shop_visibility.py`

**2. Language Switching Bug Fix**
- Root cause: `ChangeLanguageView` wasn't stripping the old language prefix from the redirect URL
- Fix: Added regex to strip `/en/`, `/de/` etc. from the current URL before passing to Django's `set_language`
- Tests in `main/tests/test_language_switching.py`

**3. Map Stone Type Filter**
- Checkbox filter UI on the main map: All / Hidden / Hunted
- JS toggles marker + polyline visibility based on `stone_type` data attribute
- `StoneWalkerStartPageView` now passes `stone_type` in the stones JSON
- Tests in `main/tests/test_map_filtering.py`

**4. Transactional Email Branding**
- All 4 email templates (activate, reset password, change email, forgotten username) branded
- Design: green header (#4CAF50), 600px max-width, white content area, branded footer
- All inline CSS for email client compatibility
- Tests in `accounts/test_email_templates.py`

**5. Menu Responsiveness Fix**
- Desktop (>1000px): nav always visible inline
- Tablet (801-1000px): nav visible with compact sizing
- Mobile (<=800px): burger menu only
- Removed ~120 lines of complex hover/click JS toggle logic
- Cleaned up ~15 duplicate CSS media query blocks

**6. QR Code Stone Numbering**
- `Stone.get_stone_number()` method returns sequential number based on creation order
- QR PNG now has green banner at top with "Stone #N" text
- Tests in `main/tests/test_stone_features.py`

**7. Stone Certificate PDF**
- New `certificate_service.py` using reportlab - professional A4 PDF
- Includes: stone image, owner name, stone number, date, QR code, branding
- Endpoint: `/stone/<pk>/certificate/` (owner-only)
- "Download Certificate" button on stone edit page
- Tests in `main/tests/test_stone_features.py`

### New Files Created
| File | Purpose |
|------|---------|
| `source/app/context_processors.py` | Shop visibility context |
| `source/main/certificate_service.py` | PDF certificate generation |
| `source/main/tests/test_shop_visibility.py` | Shop auto-hide tests |
| `source/main/tests/test_language_switching.py` | Language bug fix tests |
| `source/main/tests/test_map_filtering.py` | Map filter tests |
