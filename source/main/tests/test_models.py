...
class StoneModelTests(BaseStoneWalkerTestCase):
    ...
    def test_can_be_edited_logic(self):
        ...
        # Wandering stones cannot be edited
        wandering = self.create_stone('WANDER', status='wandering')
        self.assertFalse(wandering.can_be_edited())

    def test_can_start_wandering_logic(self):
        """Test stone wandering permission logic"""
        # Draft stones can start wandering
        draft = self.create_stone(status='draft')
        self.assertTrue(draft.can_start_wandering())

        # Published stones can start wandering
        published = self.create_stone('PUB', status='published')
        self.assertTrue(published.can_start_wandering())

        # Already wandering stones cannot start wandering again
        wandering = self.create_stone('WANDER', status='wandering')
        self.assertFalse(wandering.can_start_wandering())
    ...
    def test_can_download_qr_logic(self):
        ...
        published = self.create_stone('PUB', status='published')
        self.assertTrue(published.can_download_qr())

        wandering = self.create_stone('WANDER', status='wandering')
        self.assertFalse(wandering.can_download_qr())

    def test_can_download_certificate_logic(self):
        """Test certificate download permission logic"""
        draft = self.create_stone(status='draft')
        self.assertFalse(draft.can_download_certificate())

        published = self.create_stone('PUB', status='published')
        self.assertFalse(published.can_download_certificate())

        wandering = self.create_stone('WANDER', status='wandering')
        self.assertTrue(wandering.can_download_certificate())
    ...
    def test_start_wandering_method(self):
        """Test stone start_wandering method"""
        stone = self.create_stone(status='published')

        result = stone.start_wandering()
        self.assertTrue(result)
        self.assertEqual(stone.status, 'wandering')
        self.assertIsNotNone(stone.wandering_at)

        # Cannot start wandering on draft stone that's already wandering
        result = stone.start_wandering()
        self.assertFalse(result)

    def test_start_wandering_from_draft(self):
        """Test that draft stones can start wandering directly"""
        draft = self.create_stone('DRAFT', status='draft')
        result = draft.start_wandering()
        self.assertTrue(result)
        self.assertEqual(draft.status, 'wandering')
        self.assertIsNotNone(draft.wandering_at)
